    ...
    public sealed class KomachiModRetreat : KomachiCard
    {
        ...
        protected override IEnumerable<BattleAction> Actions(UnitSelector selector, ManaGroup consumingMana, Interaction precondition)
        {
            ...
            if (this.IsUpgraded)
            {
                ...
                {
                    if (selector.SelectedEnemy == enemyUnit && enemyUnit.IsAlive)
                    {
                        yield return KomachiDistanceSe.ChangeDistanceLevel(enemyUnit, Value1);
                    }
                    else if (enemyUnit.IsAlive)
                    {
                        yield return KomachiDistanceSe.ChangeDistanceLevel(enemyUnit, Value2);
                    }
                }
            }
            else
